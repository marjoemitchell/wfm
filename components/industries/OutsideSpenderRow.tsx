...
export type OutsideSpenderRowData = {
  slug: string;
  name: string;
  total: number;
  share: number;
  candidateCount: number;
  topCandidate: { name: string; slug: string } | null;
};

export default function OutsideSpenderRow({ row, index }: { row: OutsideSpenderRowData; index: number }) {
    ...
    >
      ...
      <div className="min-w-0">
        ...
        <div className="text-[16.2px] text-ink-tertiary">
          {row.candidateCount} {row.candidateCount === 1 ? "candidate" : "candidates"} tracked
        </div>
        <div className="mt-2 sm:hidden">
          <p className="mb-2 text-[16.2px] text-ink-secondary">Top candidate: {row.topCandidate?.name ?? "None"}</p>
      ...
      <div className="hidden sm:block">
        <p className="mb-[10px] text-[16.2px] text-ink-secondary">
          Top candidate: {row.topCandidate?.name ?? "None"}
        </p>
        <div className="h-[3px] w-full bg-track">
          <div className="h-[3px] bg-accent" style={{ width: `${Math.min(100, barPct)}%` }} />
        </div>
