...
export type BallotMeasureSpenderRowData = {
  slug: string;
  name: string;
  total: number;
  share: number;
  measureCount: number;
  topMeasure: { code: string } | null;
};

export default function BallotMeasureSpenderRow({ row, index }: { row: BallotMeasureSpenderRowData; index: number }) {
  const barPct = row.share;

  return (
    <Link
      href={`/donor/${row.slug}?from=political-spending`}
    ...
    >
      ...
      <div className="min-w-0">
        ...
        <div className="text-[16.2px] text-ink-tertiary">
          {row.measureCount} {row.measureCount === 1 ? "measure" : "measures"} tracked
        </div>
        <div className="mt-2 sm:hidden">
          <p className="mb-2 text-[16.2px] text-ink-secondary">Top measure: {row.topMeasure?.code ?? "None"}</p>
      ...
      <div className="hidden sm:block">
        <p className="mb-[10px] text-[16.2px] text-ink-secondary">Top measure: {row.topMeasure?.code ?? "None"}</p>
        <div className="h-[3px] w-full bg-track">
          <div className="h-[3px] bg-accent" style={{ width: `${Math.min(100, barPct)}%` }} />
        </div>
      </div>
