...
export default function LocationList({
  ...
  return (
    <div>
      ...
      {rows.map((r) => (
        <div key={r.label} className="py-[15px]">
          <div className="flex items-baseline justify-between">
            <span className="text-[18.6px] text-ink">
              {r.label}
            ...
            <span className="text-[19.2px] tabular-nums text-ink-secondary">{money(r.amount)}</span>
          </div>
      ...
      {footnote && (
        <p className="mt-4 flex gap-2 text-[16.2px] text-ink-faint">
